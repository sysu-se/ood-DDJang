				...
				{#each $gameDomain.grid as row, y}
					{#each row as value, x}
						<Cell {value}
						      ...
						      sameNumber={$settings.highlightSame && value && !isSelected($cursor, x, y) && getValueAtCursor($gameDomain.grid, $cursor) === value}
						      conflictingNumber={$settings.highlightConflicting && $gameDomain.givens[y][x] === false && $gameDomain.invalidCells.includes(x + ',' + y)} />
					{/each}
				{/each}
			{/if}
